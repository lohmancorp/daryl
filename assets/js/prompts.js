 ...
 * @param {boolean} shouldUpdateCount - If true, calls updateTicketCount and saveSettings. Defaults to true.
 */
function handlePromptSelection(shouldUpdateCount = true) {
    ...
    if (selectedPrompt) {
        ...
        clearPromptSelectionBtn.classList.remove('hidden');
    } else {
        currentPrompt = null;
    ...
    if (shouldUpdateCount) {
        // Re-run updateTicketCount to check start button state
        updateTicketCount();
        saveSettings();
    }
}
